import React from 'react';
import {
  AnalyticsListener,
  AnalyticsContext,
  useAnalyticsEvents,
  useCallbackWithAnalytics,
} from '@atlaskit/analytics-next';

import { JiraIcon, JiraLogo } from '@atlaskit/logo';
import Badge from '@atlaskit/badge';
import Avatar from '@atlaskit/avatar';
import {
  AtlassianNavigation,
  PrimaryButton,
  PrimaryButtonProps,
  ProductHome,
  Create,
  Search,
  Help,
  AppSwitcher,
  Settings,
  Notifications,
  Profile,
} from '../src';
import { useOverflowStatus } from '../src/controllers/overflow';
import { DropdownItem } from '@atlaskit/dropdown-menu';

const NavigationButton = ({
  name,
  ...props
}: PrimaryButtonProps & { name: string }) => {
  const { isVisible } = useOverflowStatus();
  const { createAnalyticsEvent } = useAnalyticsEvents();
  const onClick = () => {
    createAnalyticsEvent({
      action: 'clicked',
      subject: name,
    }).fire('app-navigation');
    console.log(`${name} clicked.`);
  };

  if (isVisible) {
    return <PrimaryButton {...props} onClick={onClick} />;
  } else {
    return <DropdownItem>{props.children}</DropdownItem>;
  }
};

const ProductHomeAnalytics = (onClick: any) => () => (
  <ProductHome
    icon={JiraIcon}
    logo={JiraLogo}
    onClick={onClick('product home')}
  />
);

const CreateAnalytics = (onClick: any) => () => (
  <Create text="Create" onClick={onClick('create')} />
);

const SearchAnalytics = (onClick: any) => () => (
  <Search onClick={onClick('search')} text="Search..." tooltip="Search" />
);

const AppSwitcherAnalytics = (onClick: any) => () => (
  <AppSwitcher onClick={onClick('app switcher')} tooltip="Switch to..." />
);

const HelpAnalytics = (onClick: any) => () => (
  <Help onClick={onClick('help')} tooltip="Help" />
);

const badge = () => <Badge appearance="important">3</Badge>;

const NotificationsAnalytics = (onClick: any) => () => (
  <Notifications
    onClick={onClick('notifications')}
    tooltip="Notifications"
    badge={badge}
  />
);

const SettingsAnalytics = (onClick: any) => () => (
  <Settings onClick={onClick('settings')} tooltip="Settings" />
);

const avatarUrl =
  'data:image/png;base64,';

const ProfileAnalytics = (onClick: any) => () => (
  <Profile
    icon={<Avatar src={avatarUrl} />}
    onClick={onClick('profile')}
    tooltip="Your profile and settings"
  />
);

const AnalyticsExample = () => {
  const onClick = (subject: string) => {
    return useCallbackWithAnalytics(
      () => console.log(`${subject} clicked.`),
      {
        action: 'clicked',
        subject,
      },
      'app-navigation',
    );
  };

  const primaryItemsAnalytics = () => [
    <NavigationButton name="projects">Projects</NavigationButton>,
    <NavigationButton name="filters">Filters</NavigationButton>,
    <NavigationButton name="dashboards">Dashboards</NavigationButton>,
    <NavigationButton name="apps">Apps</NavigationButton>,
  ];

  return (
    <AnalyticsListener
      channel="atlaskit"
      onEvent={event => console.log('Event fired:', event)}
    >
      <AnalyticsContext data={{ panel: 'atlassian-navigation' }}>
        <AtlassianNavigation
          primaryItems={primaryItemsAnalytics()}
          renderAppSwitcher={AppSwitcherAnalytics(onClick)}
          renderCreate={CreateAnalytics(onClick)}
          renderHelp={HelpAnalytics(onClick)}
          renderNotifications={NotificationsAnalytics(onClick)}
          renderProductHome={ProductHomeAnalytics(onClick)}
          renderProfile={ProfileAnalytics(onClick)}
          renderSearch={SearchAnalytics(onClick)}
          renderSettings={SettingsAnalytics(onClick)}
        />
      </AnalyticsContext>
    </AnalyticsListener>
  );
};

export default AnalyticsExample;
